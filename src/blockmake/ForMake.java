...
public class ForMake {	//forノードの構築
    static ForConstruction make(ArrayList<String> source,int depth){

        depth++;//新しく作るforはどこかに属しているので、深みを１上げる

        source.remove(0);//(を削除
        //ループ変数・条件式・計算式を取得
        ArrayList<String> variable = getVariable(source);
        ArrayList<String> condition = getCondition(source);
        ArrayList<String> formula = getFormula(source);

        ArrayList<String> resultSource = getProgram(source);

        BlockMake bm = BlockMake.make(resultSource, depth);

        return new ForConstruction(variable,condition,formula,bm,depth);
    }

    private static ArrayList<String> getVariable(ArrayList<String> source){
        ArrayList<String> result = getCondition(source);
